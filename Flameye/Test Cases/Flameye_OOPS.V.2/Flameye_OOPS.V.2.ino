...
const int FLAME_START_VALUE = 300; //DAC Value if flame is just on {could vary}(410)
const int FLAME_SATURATION_VALUE = 340;//DAC value of constant flame could change(450)
  ...
  public:
    ...
    void sensorSloper()
    {
      ...
      if (currentMillis - previousMillis >= OnTime)
      {
        ...
        if (newData > prevData && sensorValue > FLAME_START_VALUE && sensorValue < (FLAME_START_VALUE + 10) ) {
                  Serial.print("sensor = " );
          ...
          //Stop the Spark
        } else if (newData < prevData &&  sensorValue > FLAME_SATURATION_VALUE && turnOffCount < 6) { //Loop to detect if the flame is falling continuosly and turn off the knob
